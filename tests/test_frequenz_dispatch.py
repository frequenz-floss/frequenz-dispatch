...
async def test_multiple_dispatches_merge_running_intervals(
    ...
    dispatch1 = replace(
        ...
        target=[1, 2] if merge_strategy == MergeByType() else [3, 4],
        start_time=_now() + timedelta(seconds=5),
        recurrence=RecurrenceRule(),
        type="TEST_TYPE",
    )
...
async def test_multiple_dispatches_sequential_intervals_merge(
    ...
    dispatch1 = replace(
        ...
        target=[3, 4] if merge_strategy == MergeByType() else [1, 2],
        start_time=_now() + timedelta(seconds=5),
        recurrence=RecurrenceRule(),
        type="TEST_TYPE",
    )
